...
function getTheme(style,brandOn,brandColors){
  if(brandOn&&brandColors)return{dark:toHex(brandColors.primary||"#1E2761"),light:"FFFFFF",text:toHex(brandColors.text||"#1A2320"),accent:toHex(brandColors.accent||"#4472C4")};
  ...
  for(let si=0;si<slides.length;si++){
    ...
    for(const el of(sd.elements||[])){
      ...
      if(el.type==="text"){
        ...
        s.addShape(pres.shapes.LINE,{x,y,w,h,line:{color:toHex(el.stroke||"#333333"),pt:Math.max(1,Math.round((el.strokeWidth||2)*0.75))}});
      }else if(el.type==="star"){
        s.addShape(pres.shapes.STAR_5,{x,y,w,h,fill:{color:toHex(el.fill||"#cccccc")},line:el.stroke&&el.stroke!=="transparent"?{color:toHex(el.stroke),pt:Math.max(1,Math.round((el.strokeWidth||2)*0.75))}:{type:"none"}});
      }
    }
  }
}
module.exports=async function handler(req,res){
  ...
  try{
    ...
    if(action==="outline"){
      ...
      const prompt=`You are an expert presentation consultant and presentation layout engine. Create a ${slideCount}-slide presentation.
USER INPUT:
${input}

STRICT RULES:
- Return ONLY a raw JSON array of exactly ${slideCount} slide objects. No markdown, no fences.
- Keep every slide visually sparse and readable.
- Headings: max 8 words.
- Subheadings: max 14 words.
- Bullets: max 4 bullets on standard slides, max 3 on image or statistic slides.
- Each bullet: max 10 words and one idea only.
- Paragraphs: max 24 words. Omit if not needed.
- Use short phrases instead of long sentences.
- First slide layout must be "cover-center", dark:true.
- Last slide layout must be "closing", dark:true.
- Mix layouts intelligently across the deck.
- Never pack too much text into comparison, image, timeline, stats, or pyramid layouts.
- For these layouts use these content caps:
  * cover-center/cover-split/cover-circle/cover-dark: heading + short subheading + optional 1 line paragraph only
  * default/title-body: up to 4 bullets OR 1 short paragraph
  * two-col/comparison: up to 6 short bullets total
  * three-col/three-stats/three-images: exactly 3 short items
  * stat/big-statement: 1 big stat + up to 2 short bullets
  * timeline/process/pyramid/four-icons: exactly 4 short items
  * two-icons/two-images: exactly 2 short items
  * img-right/img-left/img-top/img-full/img-mosaic/img-hero: very light text, prioritize image space
- Prefer concrete phrases and numbers, but keep wording compact.

Each object must have ALL these exact fields:
{"title":"3-6 words","type":"title|content|data|quote|cta","dark":true,"layout":"cover-center|cover-split|cover-circle|cover-dark|default|two-col|three-col|title-body|quote|big-statement|agenda|closing|stat|three-stats|timeline|four-icons|two-icons|comparison|process|pyramid|img-right|img-left|img-full|img-top|two-images|three-images|img-mosaic|img-hero","heading":"specific headline","subheading":"","bullets":["Short point one","Short point two","Short point three","Short point four"],"paragraph":"Short supporting line.","stat":"","quote":"","author":"","imageKeyword":"3-5 words","speakerNote":"one sentence"};`
