...
// Returns 6-char hex string, or null for transparent/invisible colours
function toHex(c){
...
async function addLogo(s,pres,data,pos,wb,isCover){
  ...
  if(wb){
    ...
    s.addShape(pres.shapes.RECTANGLE,{x:0,y:isBot?5.625-bh:0,w:10,h:bh,fill:{color:"FFFFFF"},line:{color:"FFFFFF"}});
  }
  // PptxGenJS expects "mediaType;base64,data" without leading "data:"
  const imgData=data.startsWith("data:")?data.slice(5):data;
  try{s.addImage({data:imgData,x,y,w:fw,h:fh});}catch(e){console.error("addLogo:",e.message);}
}
    ...
    for(const el of(sd.elements||[])){
      ...
      if(el.type==="text"){
        ...
        const baseOpts={x,y,w,h,fontFace:el.fontFamily||"Calibri",bold:!!el.bold,italic:!!el.italic,
          fontSize:pt,color:colHex,align:el.align||"left",valign:"top",wrap:true,margin:[2,4,2,4]};
        if(raw.includes("\n")){
          const lines=raw.split("\n");
          s.addText(lines.map((ln,j)=>({text:ln,options:{
        ...
        if(!fill)continue; // skip transparent shapes
        ...
        if(!cfHex&&!csHex)continue; // fully invisible, skip
        const cf=cfHex?{color:cfHex}:{type:"none"};
        const cs=csHex?{color:csHex,pt:Math.max(1,Math.round((el.strokeWidth||2)*0.75))}:{type:"none"};
